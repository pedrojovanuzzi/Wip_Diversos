...
import { FaPlugCirclePlus, FaPix } from "react-icons/fa6";
  ...
  return (
    ...
    >
      ...
      {mounted &&
        ...
        createPortal(
          <div
            className="fixed p-3 bg-stone-900 border border-stone-700 text-white text-sm rounded-md shadow-xl z-[9999] pointer-events-none transition-all duration-200 animate-in fade-in zoom-in-95"
            style={{
              top: tooltipPos.top,
...
export default function NavBar({
  ...
  // Tudo que depende de window é resolvido após mount, evitando hydration mismatch.
  useEffect(() => {
  ...
  // Logout: dispara o Server Action (apaga cookie e redireciona pra /auth/login)
  const handleLogout = () => {
    void logoutAction();
  };
          ...
          <ul className="grid grid-cols-2 gap-5 p-4">
            {permission >= 2 && (
              <>
                <NavItem
                  to="/"
                  icon={<HiHome className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Início"
                  description="Página inicial do sistema"
                />
                <NavItem
                  to="/Pix"
                  icon={<FaPix className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Pix"
                  description="Gestão de pagamentos via Pix"
                />
                <NavItem
                  to="/solicitacoes-servico"
                  icon={<FaList className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Serviços Solicitados"
                  description="Ver solicitações de serviços"
                />
                <NavItem
                  to="/chamados/ficha-tecnica"
                  icon={<FaToolbox className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Ficha Técnica"
                  description="Fichas técnicas de chamados em campo"
                />
                <NavItem
                  to="/phone-location"
                  icon={<FaLocationArrow className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Localização"
            ...
            {permission >= 5 && (
              <>
                <NavItem
                  to="/zapsign-config"
                  icon={<FaFilePdf className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Config ZapSign"
                  description="Configurar templates ZapSign"
                />
                {isLocalhost && permission >= 5 && (
                  <NavItem
                    to="/zapsign-teste"
                    icon={<FaFlask className="text-white size-8 transition-all hover:text-amber-400" />}
                    title="Teste Docs"
                ...
                <NavItem
                  to="/Create"
                  icon={<FaUserPlus className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Novo Usuário"
                  description="Cadastrar novos usuários no sistema"
                />
                <NavItem
                  to="/licencas"
                  icon={<FaDesktop className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Licenças"
                  description="Gerenciamento de licenças de software"
                />
                <NavItem
                  to="/TimeTracking/Admin"
                  icon={<FaUsers className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Gestão de Ponto"
                  description="Administração de registros de ponto"
                />
                <NavItem
                  to="/TimeTracking/Map"
                  icon={<FaMapMarkedAlt className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Mapa"
                  description="Visualização de localização em tempo real"
                />
                <NavItem
                  to="/TimeTracking/Report"
                  icon={<FaClipboardList className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Relatórios"
                  description="Relatórios detalhados de ponto"
                />
                <NavItem
                  to="/whatsapp/broadcast"
                  icon={<BsFillSendPlusFill className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Disparo em Massa"
                  description="Enviar mensagens para múltiplos clientes"
                />
                <NavItem
                  to="/Pm2Logs"
                  icon={<FaTerminal className="text-white size-8 transition-all hover:text-green-400" />}
                  title="PM2 Logs"
            ...
            {permission >= 2 && (
              <>
                <NavItem
                  to="/feedbackCreate"
                  icon={<MdOutlineFeedback className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Feedback"
                  description="Enviar sugestões ou reportar erros"
                />
                <NavItem
                  to="/NFSE"
                  icon={<HiDocumentText className="text-white size-8 transition-all hover:text-green-400" />}
                  title="NFSe"
                  description="Emissão e gestão de Nota Fiscal de Serviço"
                />
                <NavItem
                  to="/Nfcom"
                  icon={<FaFileInvoice className="text-white size-8 transition-all hover:text-green-400" />}
                  title="NFCom"
                  description="Emissão e gestão de Nota Fiscal de Telecom"
                />
                <NavItem
                  to="/nfe/comodato"
                  icon={<GrDocumentNotes className="text-white size-8 transition-all hover:text-green-400" />}
                  title="NFe Comodato"
                  description="Remessa e retorno de equipamentos"
                />
                <NavItem
                  to="/Whatsapp"
                  icon={<FaWhatsapp className="text-white size-8 transition-all hover:text-green-400" />}
                  title="WhatsApp"
                  description="Integração e envio de mensagens"
                />
                <NavItem
                  to="/ClientAnalytics"
                  icon={<IoMdAnalytics className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Analytics"
                  description="Análise de dados de clientes"
                />
                <NavItem
                  to="/grafico-instalacoes"
                  icon={<FaChartLine className="text-white size-8 transition-all hover:text-green-400" />}
            ...
            {permission === 1 && (
              <>
                <NavItem
                  to="/Nfcom/Buscar"
                  icon={<FaSearch className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Buscar NFCom"
                  description="Pesquisar notas fiscais"
                />
                <NavItem
                  onClick={handleLogout}
                  icon={<ImExit className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Sair"
            ...
            <NavItem
              ...
              icon={<FaClock className="text-white size-8 transition-all hover:text-green-400" />}
            ...
            {permission >= 2 && (
              <>
                <NavItem
                  to="/ServerLogs"
                  icon={<FaRegFolder className="text-white size-8 transition-all hover:text-green-400" />}
                  title="Logs"
                  description="Visualizar logs do servidor"
                />
                <NavItem
                  to="/PowerDns"
                  icon={<MdDns className="text-white size-8 transition-all hover:text-green-400" />}
                  title="DNS"
                  description="Gerenciamento de PowerDNS"
                />
                <NavItem
                  to="/Onu"
                  icon={<FaPlugCirclePlus className="text-white size-8 transition-all hover:text-green-400" />}
                  title="ONU"
