...
export default function PrefeituraLogin() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.post(
          `${process.env.REACT_APP_URL}/Prefeitura/redirect_2`,
          {
  ...
  useEffect(() => {
    const newOtp = Array.from({ length: 8 }, () =>
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_URL}/Prefeitura/Login`,
        {
          name: data.get("name"),
  ...
  return (
    <>
      ...
      <div className="relative p-10 mt-10">
        <footer className="absolute mb-1 left-1/2 -translate-x-1/2 bottom-0 text-gray-400 text-sm">
          © 2025 Prefeitura de Arealva, Wip Telecom Multimia Eirelli, Pedro
          Artur Jovanuzzi.
        </footer>
      </div>
    </>
